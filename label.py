...
def label_trades(
    ...
    # Use 0 to denote bars with no valid signal (fixed from -1)
    df["long_label"] = np.where(cond_long, 1, 0)
    df["short_label"] = np.where(cond_short, 1, 0)
    ...
    if train_end_date:
        ...
        df.loc[df.index[mask], ["long_label", "short_label"]] = 0
