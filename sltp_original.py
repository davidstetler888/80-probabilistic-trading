...
def find_best_bucket(
    ...
    """Find the best performing bucket for a signal using only information available at signal time.
    ...
    # Sort by risk-reward ratio (descending) and take the first one
    best_bucket = sorted(bucket_stats, key=lambda x: x['rr_ratio'], reverse=True)[0]
