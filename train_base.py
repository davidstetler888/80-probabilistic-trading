...
def train_with_quality_focus():
    ...
    long_params = {
        ...
        'boosting_type': 'gbdt',
        'num_leaves': 31,  # Increased from 15
        ...
        'reg_alpha': 0.1,  # Reduced from 0.3
        'reg_lambda': 0.1,  # Reduced from 0.3
        'scale_pos_weight': 3  # Reduced from 5
    }
    ...
    for i, (idx, long_pred, short_pred) in enumerate(zip(X.index, long_predictions, short_predictions)):
        # Use lower thresholds to generate more signals
        if long_pred > 0.3 and long_pred > short_pred + 0.02:  # Lowered from 0.5 and 0.05
            signals.append({
                'timestamp': idx,
                'side': 'long',
        ...
        elif short_pred > 0.3 and short_pred > long_pred + 0.02:  # Lowered from 0.5 and 0.05
            signals.append({
                'timestamp': idx,
                'side': 'short',
    ...
    if len(signals_df) > 0:
        ...
        # Generate probs_base.csv for subsequent scripts
        probs_df = pd.DataFrame(index=X.index)
        probs_df['long_prob'] = long_predictions
        probs_df['short_prob'] = short_predictions
        probs_df.to_csv(run_dir / "data" / "probs_base.csv")
        ...
        print("✅ Generated probs_base.csv for subsequent scripts")
        return True
    else:
...
def main():
    ...
    return train_with_quality_focus()
